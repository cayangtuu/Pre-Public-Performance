...
for var in vars:
    ...
    allData = pd.read_csv(inDir + '/201812' + var + '.csv',
                           encoding='utf-8-sig', index_col=0)

    for tt in range(12):
        data = pd.read_csv(inDir + '/2019' + '%02d' % (tt+1) + var + '.csv',
                           encoding='big5', index_col=0)
        Data = pd.DataFrame()
        for st in ssList:
    ...
    ### LTS -> UTC
    allData.index = pd.date_range('2018-12-31-01', '2020-01-02-00', freq='1h') \
                    -pd.Timedelta('8h')
