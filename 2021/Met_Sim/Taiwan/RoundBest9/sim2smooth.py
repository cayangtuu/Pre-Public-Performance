...
def main():
    ...
    Start = datetime.datetime.strptime('2021-06-29', '%Y-%m-%d')
    End = datetime.datetime.strptime('2021-07-01', '%Y-%m-%d')
...
def simvsobs(dfcom, var, filetime):
    obsDir = os.path.join('/home/cayang/DataProcess/2021/Met_Obs/Taiwan/data_out', var)
    obsFil = os.path.join(obsDir, filetime+'_'+var+'_obs.csv')
    obsdf = pd.read_csv(obsFil, index_col=0)
...
def smooth_raw(ds_1st, ds_2nd, LastSet, filetime):
    ...
    if (filetime == '2018-12-31'):
       OutDir = os.path.join(rootDir, 'data_out', 'smooth_raw', '2019-01')
    else:
       OutDir = os.path.join(rootDir, 'data_out', 'smooth_raw', filetime[:7])
    try:
       os.makedirs(OutDir)
